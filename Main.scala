...
class InputHandler{
  ...
  def introduction: Unit = {
    println("Hello")
    println("This program takes in an BMP image and produces it's negative.")
    println("Or you can experiment with Grayscale feature, which is under construction")
...
//      }
    ...
    //step 4: Tell each pixel to negative itself
      /**
       * Produces a new Array[Byte], by negating all pixels starting at the index
       * of bitmap, i.e. RGB info
       * @param byteArrr
       * @return Array[Byte] --
       */
      def makeAllPixelsAfterStartPixelDataNegative(byteArrr:  Array[Byte]): Array[Byte] = {
        byteArrr.zipWithIndex.map {
          case (element, index) if index < startPixelData.toInt => element
          case (element, index) if index >= startPixelData.toInt => (255 - byteToUnsignedInt(element)).toByte
        }
      }

      /**
       * This checks if an index needs to be greyscaled.
       * Place stands for either 0, 1 2.
       * zero is the fist color.
       * one is the second color.....
       * returns Boolean
       * @param index
       * @param place
       * @return Boolean
       */
      def isANeedToGreyIndex(index:Int, place:Int): Boolean ={
        val lessThan = index >= startPixelData.toInt
        val indexInThatValue = ((index - 54 ) -( pixelRowLengthWithPadding * Math.floor((index - 54)/ pixelRowLengthWithPadding ).toInt))
        val notPaddingPixel = indexInThatValue <= pixelRowLengthOfInfo
        val isDivisbleByThree = indexInThatValue % 3 == place
        //if (index == 54 || index == 55 || index == 56) println( index.toString + lessThan + " " + notPaddingPixel.toString + " " + isDivisbleByThree.toString + " "+ indexInThatValue.toString)
        lessThan && notPaddingPixel && isDivisbleByThree
      }
       ...
       * @return Array[Byte]
       */
      def makeAllPixelsAfterStartPixelDataGresyscale(byteArrr:  Array[Byte]): Array[Byte] = {
        byteArrr.zipWithIndex.map {
          case (element, index) if index < startPixelData.toInt => element
          case (element, index) if isANeedToGreyIndex(index , 0) => (byteArrr.slice(index, index+3).map(x => x.toInt).sum/3).toByte // .reduce( (x, y) => (x.toInt + y.toInt).toByte ).toInt/3).toByte
          case (element, index) if isANeedToGreyIndex(index , 1) => byteArrr(index-1)
          case (element, index) if isANeedToGreyIndex(index , 2) => byteArrr(index-2)
          case (element, index) => byteArrr(index-2)
        }
      }


      //Map of functions to match, keys are proccess type(grayscale or negative) value is a function associated with function
      val mapOfPixelAreaTransformationFunctions = Map[String, (Array[Byte]) => Array[Byte]]("-g" -> makeAllPixelsAfterStartPixelDataGresyscale _ , "-n" -> makeAllPixelsAfterStartPixelDataNegative _ )

      //New transformed byte array
      val trasnformedRGBByteArray =  mapOfPixelAreaTransformationFunctions(processType)(byteArray)

      //STEP 5: Write to file
      write(fileName+ new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(new Date())+processType+".BMP", trasnformedRGBByteArray)
